...
//                        IjkMediaPlayer.toggleDotPort(pos > 0);
        ...
        findViewById(R.id.m3u8Ad).setOnClickListener(v -> {
            FastClickCheckUtil.check(v);
        ...
        //下次进入
        findViewById(R.id.tvHomeLive).setOnClickListener(v -> {
            FastClickCheckUtil.check(v);
            Hawk.put(HawkConfig.DEFAULT_LOAD_LIVE, !Hawk.get(HawkConfig.DEFAULT_LOAD_LIVE, false));
            tvHomeDefaultShow.setText(Hawk.get(HawkConfig.DEFAULT_LOAD_LIVE, false) ? "直播" : "点播");
        });

        findViewById(R.id.llIjkCachePlay).setOnClickListener((this::onClickIjkCachePlay));
        findViewById(R.id.llClearCache).setOnClickListener((this::onClickClearCache));
    }
